...
import { isValidAnalyticsTopic, readAnalytics, type Topic } from '$lib/models/analytics'
...
export const load: PageServerLoad<{ topic: Topic, year: number, minutes: number[] }> = async ({ params, url }) => {
  ...
  if (isValidAnalyticsTopic(topic)) {
    const data = await readAnalytics(topic,
      new Date(Date.UTC(year, 0, 1, 0, 0, 0, 0)),
      new Date(Date.UTC(year + 1, 0, 1, 0, 0, 0, 0) - 1)
    )
